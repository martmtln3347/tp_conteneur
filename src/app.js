...
app.use((req, res, next) => {
  ...
  console.log(`[${ts()}] -> ${method} ${url} (received)`);
  ...
  res.on('finish', () => {
    ...
    console.log(
      `[${ts()}] <- ${method} ${url} (status=${res.statusCode}) (durationMs=${durationMs.toFixed(2)})`
    );
    console.log(`[${ts()}] counter processedRequests=${processedRequests}`);
  });
